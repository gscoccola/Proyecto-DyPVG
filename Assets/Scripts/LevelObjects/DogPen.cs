...
public class DogPen : MonoBehaviour
{
    ...
    private IEnumerator TriggerWinCondition()
    {
        ...
        int previousScore = PersistentInfo.Instance.CompletionInfo[SceneManager.GetActiveScene().buildIndex - 2].Score;
        if (TurnManager.Instance.CurrentTurnIndex < previousScore ||
            previousScore == 0)
            PersistentInfo.Instance.CompletionInfo[SceneManager.GetActiveScene().buildIndex - 2].Score = TurnManager.Instance.CurrentTurnIndex;
